...
- Routes: `/login` (sign in / sign up), `/perfil` (body data → calorie goal; first stop after sign-up), `/` (daily goal card + meal plan), `/nutrientes` (macro/vitamin/mineral progress cards), `/historico` (7/30/90-day calorie balance, activity data shaped like Google Fit / Apple Saúde exports, past meal plans; components in `src/components/history/`) and `/chat` (mocked AI chat). Page files are thin — they compose `AuthGuard`, the shared `Topbar` and page-specific components.
